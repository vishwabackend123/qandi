...
<script type="text/javascript">
    ...
    if (performance.navigation.type == performance.navigation.TYPE_RELOAD) {
        ...
        window.location = "{{url('live_exam_list')}}";
    } else {
                            ...
                            <div class="examTabheader">
                                <div class="tablist">
                                    <ul class="nav nav-tabs mobilescrolltab" role="tablist" id="myTab">
                                        ...
                                        <li class="nav-item">
                                            <a class="nav-link qq1_2_3_4 all_div class_{{$sub->id}} @if($activesub_id==$sub->id) active @endif " id="{{$sub->subject_name}}-tab" data-bs-toggle="tab" href="#{{$sub->subject_name}}" @if(count($filtered_subject)>1) onclick="get_subject_question('{{$sub->id}}')" @endif >{{$sub->subject_name}} <span class="qCount">{{$sub->count}}</span></a>
                                        </li>
                                        @endforeach
                                        @endif
                                    </ul>
                ...
                <div class="modal-body">
                    ...
                    <div class="exam-footer-sec">
                        <div class="task-btn tasklistbtn">
                            ...
                            <button class="btn btn-common-green submitPopupBtn"> Submit Test <label><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M16.95 7.767 5.284 1.934a2.5 2.5 0 0 0-3.4 3.25l2 4.475a.883.883 0 0 1 0 .683l-2 4.475a2.5 2.5 0 0 0 2.283 3.517c.39-.004.774-.095 1.125-.267l11.667-5.833a2.5 2.5 0 0 0 0-4.467h-.009zm-.741 2.975L4.542 16.575a.833.833 0 0 1-1.125-1.083l1.992-4.475c.025-.06.048-.12.066-.183h5.742a.833.833 0 0 0 0-1.667H5.475a1.668 1.668 0 0 0-.066-.183L3.417 4.509a.833.833 0 0 1 1.125-1.084L16.209 9.26a.834.834 0 0 1 0 1.483z" fill="#fff" />
                                    </svg>
                                </label>
                            </button>
...
<script type="text/javascript">
    ...
    function qnext(question_id) {
        ...
        url = "{{ url('live_next_question/') }}/" + question_id;
        $.ajax({
            url: url,
            data: {
                "_token": "{{ csrf_token() }}",
